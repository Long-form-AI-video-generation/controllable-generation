...
class DepthOnlyProcessor:
    """Process control NPZ files — depth only, no encoder (raw MiDaS values)"""
    ...
    def _prepare_depth_raw(self, depth: np.ndarray) -> np.ndarray:
        """
        Raw MiDaS depth: (T, H, W) uint8 0-255
        → (1, 256, T, 128, 128) float16

        No encoder — just resize, normalize, expand channels.
        Real near/far values preserved.
        """
        depth = self._sample_frames(depth, self.num_frames)  # (T, H, W)

        # Resize each frame
        W, H = self.resolution
        resized = []
        for t in range(depth.shape[0]):
            frame = cv2.resize(depth[t], (128, 128), interpolation=cv2.INTER_LINEAR)
            resized.append(frame)
        depth = np.stack(resized)  # (T, 128, 128)

        # Normalize to [0, 1]
        depth = depth.astype(np.float32) / 255.0  # (T, 128, 128)

        # (T, 128, 128) → (1, 1, T, 128, 128) tensor
        tensor = torch.from_numpy(depth).unsqueeze(0).unsqueeze(0)  # (1, 1, T, 128, 128)
    ...
    def process_single_file(self, file_info: dict) -> dict:
        ...
        try:
            ...
            if 'depth' not in data:
                result['errors'].append("no depth key in npz")
                return result

            depth_encoded = self._prepare_depth_raw(data['depth'])
    ...
    def process_all(self):
        print("=" * 70)
        print(f"Depth-Only Encoding (raw MiDaS, no encoder)")
        ...
        for file_info in tqdm(self.npz_files, desc="Encoding depth", ncols=80):
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Encode depth controls (raw MiDaS, no encoder)")
    ...
    parser.add_argument('--output_dir', type=str,
                        ...
                        help='Output directory for encoded depth NPZ files')
    parser.add_argument('--num_frames', type=int, default=8)
    args = parser.parse_args()

    processor = DepthOnlyProcessor(
