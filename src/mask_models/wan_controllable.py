...
from .control_adapter import MaskControlAdapter
...
class ControllableWAN(DepthControllableWAN):
    """WAN 2.2 with a checkpointed semantic mask condition path."""
    ...
    def __init__(
        ...
        *,
        ...
        self.control_adapter = MaskControlAdapter(
            hidden_dim=512,
            dit_dim=dit_dim,
            condition_dim=256,
            target_spatial=16,
    ...
    def train(self, mode: bool = True):
        """Train mask modules while keeping every pretrained component in eval."""
        super().train(mode)
    ...
    def _control_injection_hook(self, module, inputs):
        ...
        if source_tokens.shape[1] % (16 * 16) != 0:
            raise RuntimeError(
                f"invalid mask token count: {source_tokens.shape[1]}"
            )
