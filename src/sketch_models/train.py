"""Train the sketch-only WAN control adapter.
...
from src.sketch_models.training_schedule import expected_optimizer_steps
...
class SketchTrainer(MultiVideoTrainer):
        ...
        for batch_index, batch in enumerate(progress):
            try:
                ...
                raise RuntimeError(
                    ...
                    "strict sketch training does not silently skip samples"
                )
    ...
    def export_for_inference(self, name: str = "final") -> None:
        ...
        adapter_path = self.checkpoint_dir / f"sketch_adapter_{name}.safetensors"
        zero_convs_path = (
            self.checkpoint_dir / f"sketch_zero_convs_{name}.safetensors"
        )
    ...
    def load_checkpoint(self, checkpoint_path: str) -> None:
        ...
        if actual != expected:
            raise ValueError(
                "Checkpoint is not compatible with this sketch model. "
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("Sketch training requires CUDA")
    ...
    if preprocessing_manifest.get("control_key") != CONTROL_KEY:
        raise ValueError(
            "Preprocessing manifest is not a sketch_encoded dataset"
        )
