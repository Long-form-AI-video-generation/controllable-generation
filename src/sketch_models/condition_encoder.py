"""Trainable spatio-temporal encoder for sparse Canny edge sequences."""
...
class SketchConditionEncoder(nn.Module):
    """Map ``[B,1,T,H,W]`` Canny controls to ``[B,256,T,16,16]``."""
    ...
    @staticmethod
    def _validate_input(control: torch.Tensor) -> None:
        if control.ndim != 5:
            raise ValueError(
                f"expected sketch control [B,1,T,H,W], got {tuple(control.shape)}"
            )
        if control.shape[1] != 1:
            raise ValueError(
                f"expected one sketch channel, got {control.shape[1]}"
            )
        if control.shape[2] <= 0 or control.shape[3] <= 0 or control.shape[4] <= 0:
            raise ValueError("sketch control dimensions must be non-empty")
        if not torch.isfinite(control).all():
            raise ValueError("sketch control contains NaN or Inf")
        minimum = float(control.detach().amin())
        maximum = float(control.detach().amax())
        if minimum < 0.0 or maximum > 1.0:
            raise ValueError(
                f"sketch control must be in [0,1], got [{minimum}, {maximum}]"
            )
