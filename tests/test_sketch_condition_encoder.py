...
if torch is not None:
    from src.sketch_models.condition_encoder import (
        SketchConditionEncoder,
        TemporalRefinement,
    )


@unittest.skipIf(torch is None, "PyTorch is not installed in this local environment")
class SketchConditionEncoderTests(unittest.TestCase):
    def test_shape_and_backward(self):
        torch.manual_seed(7)
        encoder = SketchConditionEncoder(out_channels=32, target_spatial=16)
        control = torch.zeros(2, 1, 4, 128, 128)
        control[:, :, :, 24:104, 63:65] = 1.0
    ...
    def test_line_changes_spatial_features(self):
        ...
        encoder = SketchConditionEncoder(out_channels=32, target_spatial=16).eval()
        blank = torch.zeros(1, 1, 2, 128, 128)
        line = blank.clone()
        line[:, :, :, 16:112, 63:66] = 1.0
        with torch.no_grad():
    ...
    def test_invalid_channel_count_is_rejected(self):
        encoder = SketchConditionEncoder(out_channels=32)
        with self.assertRaises(ValueError):
            encoder(torch.zeros(1, 2, 2, 32, 32))
